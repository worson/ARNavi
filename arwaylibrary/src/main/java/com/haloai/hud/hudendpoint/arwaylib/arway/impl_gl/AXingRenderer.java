...
public class AXingRenderer extends Renderer implements IAnimationListener, IRenderStrategy.RenderParamsNotifier {
    ...
    private static final double ROAD_WIDTH        = 10;//ARWayProjection.ROAD_WIDTH/*Math.tan(Math.toRadians(22.5))*2*400/280 * 0.5*/ /*ARWayConst.ROAD_WIDTH*/;
    ...
    public void initScene() {
        HaloLogger.logE(ARWayConst.ERROR_LOG_TAG, "ARRender reset called!");
    ...
    private double getCenterAndNext(int naviIcon, Vector3 center, Vector3 next, Vector3 fin, List<Vector3> leftPath, List<Vector3> rightPath, List<Vector3> intersectPath, double length) {
        ...
        for (int i = mCurIndexInPath + 1; i < mPath.size(); i++) {
            ...
            if (dist >= dist300) {
                ...
                // TODO: 2016/8/3 翻转从主路copy出来的一段路,作为岔路显示,效果可能会更加自然,因此上述代码是不起作用的
    ...
    /**
     * clear not use data after we reset path data!
     */
